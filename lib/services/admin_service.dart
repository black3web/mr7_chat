...
class AdminService {
  ...
  Future<Map<String, dynamic>> getStatistics() async {
    ...
    try {
      final usersSnap = await _firestore.collection(AppConstants.usersCollection).count().get();
    ...
    try {
      final groupsSnap = await _firestore.collection(AppConstants.groupsCollection).count().get();
    ...
    try {
      final aiLogsSnap = await _firestore.collection(AppConstants.aiLogsCollection).count().get();
    ...
    try {
      final supportSnap = await _firestore.collection(AppConstants.supportCollection).count().get();
    ...
    try {
      final aiLogs = await _firestore.collection(AppConstants.aiLogsCollection).get();
  ...
  Future<void> sendBroadcast({
    ...
    await _firestore.collection(AppConstants.broadcastsCollection).add({
      'title': title,
      'message': message,
      'link': link,
      'senderId': senderId,
  ...
  Future<void> dismissBroadcast(String broadcastId, String userId) async {
    await _firestore.collection(AppConstants.broadcastsCollection).doc(broadcastId).update({
  ...
  Future<void> replyToSupport({
    ...
    await _firestore.collection(AppConstants.supportCollection).doc(threadId).update({
      'hasUnreadReply': true,
  ...
  Future<Map<String, dynamic>?> getUserDetails(String userId) async {
    ...
    final aiLogs = await _firestore
        .collection(AppConstants.aiLogsCollection)
  ...
  Future<List<UserModel>> searchUsers(String query) async {
    ...
    if (RegExp(r'^\d+$').hasMatch(query)) {
      try {
        final doc = await _firestore.collection(AppConstants.usersCollection).doc(query).get();
