...
class GroupService {
  ...
  Future<GroupModel> createGroup({
    ...
    await _firestore.collection(AppConstants.groupsCollection).doc(groupId).set(group.toMap());
  ...
  Future<GroupModel?> joinGroupByUsername(String username, String userId) async {
    ...
    await _firestore.collection(AppConstants.groupsCollection).doc(group.id).update({
  ...
  Stream<List<GroupModel>> listenToUserGroups(String userId) {
    return _firestore
        .collection(AppConstants.groupsCollection)
        .where('members', arrayContains: null)
        .snapshots()
        .map((_) => <GroupModel>[]);
    // Note: Firestore doesn't support querying nested arrays directly
    // This is handled by storing memberIds as a separate field
  }
  ...
  Future<void> deleteGroup(String groupId) async {
    ...
    batch.delete(_firestore.collection(AppConstants.groupsCollection).doc(groupId));
    await batch.commit();
  }
