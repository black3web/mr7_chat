...
class StoryService {
  ...
  Future<StoryModel> addStory({
    ...
    await _firestore.collection(AppConstants.storiesCollection).doc(storyId).set(story.toMap());
    return story;
  }
  ...
  Future<void> viewStory(String storyId, String viewerId) async {
    final storyRef = _firestore.collection(AppConstants.storiesCollection).doc(storyId);
  ...
  Future<List<StoryView>> getStoryViewers(String storyId) async {
    final doc = await _firestore.collection(AppConstants.storiesCollection).doc(storyId).get();
    if (!doc.exists) return [];
    final story = StoryModel.fromMap(doc.data()!);
    return story.views;
  }
