...
class AuthService {
  ...
  String? validateUsername(String username) {
    ...
    if (username.length < AppConstants.minUsernameLength) return 'usernameTooShort';
    if (username.length > AppConstants.maxUsernameLength) return 'usernameTooLong';
    final regex = RegExp(AppConstants.usernameRegex);
  ...
  String? validatePassword(String password) {
    ...
    if (password.length < AppConstants.minPasswordLength) return 'passwordTooShort';
    if (password.length > AppConstants.maxPasswordLength) return 'passwordTooLong';
    return null;
  }
  ...
  Future<void> initDevAccount() async {
    ...
    if (existing.docs.isEmpty) {
      final devUser = UserModel(
        ...
        passwordHash: AppConstants.devPassword, // Already hashed
  ...
  Future<void> logout() async {
    if (_currentUser != null) {
      await _firestore.collection(AppConstants.usersCollection).doc(_currentUser!.id).update({
        'isOnline': false,
        'lastSeen': Timestamp.fromDate(DateTime.now()),
      });
    }
  ...
  Future<void> updateProfile({
    ...
    await _firestore.collection(AppConstants.usersCollection).doc(_currentUser!.id).update(updates);

    final doc = await _firestore.collection(AppConstants.usersCollection).doc(_currentUser!.id).get();
    _currentUser = UserModel.fromMap(doc.data()!);
  }
  ...
  Future<void> deleteAccount() async {
    if (_currentUser == null) return;
    await _firestore.collection(AppConstants.usersCollection).doc(_currentUser!.id).delete();
    await logout();
  }
}
