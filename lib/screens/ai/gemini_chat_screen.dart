    ...
    return Scaffold(
      ...
      body: Container(
        ...
        child: SafeArea(child: Column(children: [
          _AiHeader(title: 'Gemini 2.5 Flash', subtitle: 'Google AI', color: const Color(0xFF4285F4), icon: Icons.auto_awesome_rounded),
          Expanded(
...
class _DeepSeekChatScreenState extends State<DeepSeekChatScreen> {
  ...
  void initState() {
    super.initState();
    _messages.add(_AiMsg(text: 'مرحبا! انا DeepSeek. يمكنك الاختيار بين نماذج متعددة. كيف يمكنني مساعدتك؟', isUser: false, time: DateTime.now()));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Container(
        decoration: const BoxDecoration(gradient: AppGradients.backgroundGradient),
        child: SafeArea(child: Column(children: [
          _AiHeader(title: 'DeepSeek', subtitle: _modelNames[_model] ?? 'DeepSeek', color: const Color(0xFF00BCD4), icon: Icons.auto_awesome_rounded),
          // Model selector
          SingleChildScrollView(
            ...
            child: Row(children: _modelNames.entries.map((e) => Padding(
              ...
              child: GestureDetector(
                onTap: () { setState(() { _model = e.key; _conversationId = null; _messages.clear(); _messages.add(_AiMsg(text: 'تم تغيير النموذج الى ${e.value}', isUser: false, time: DateTime.now())); }); },
                child: Container(
